...
double NISTCalcVoltage(byte tc_type, double temp)
{
	byte nCoef;
	byte index;
	double fVoltage = 0.0;
...
double NISTCalcTemp(byte tc_type, double voltage)
{
	byte index;
	byte num;
	byte mytable;
	...
	while ((index < num) &&
		(voltage > ThermocoupleData[tc_type].Tables[index].VThreshold))
	{
		index++;
	}
