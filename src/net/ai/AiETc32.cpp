...
AiETc32::AiETc32(const NetDaqDevice& daqDevice) : AiNetBase(daqDevice)
{
...
void AiETc32::tIn(int channel, TempScale scale, TInFlag flags, double* data)
{
	tInArray(channel, channel, scale, (TInArrayFlag) flags, data);
}
void AiETc32::tInArray(int lowChan, int highChan, TempScale scale, TInArrayFlag flags, double data[])
{
...
#pragma pack(1)
	struct
	{
		unsigned char wait;
		unsigned char unit;
		unsigned int  chan_mask_base;
		unsigned int  chan_mask_exp;
	} params;
#pragma pack()

	memset(&params, 0, sizeof(params));

	params.wait = waitForNewData;
	params.unit = units;

	for(int ch = lowChan; ch <= highChan; ch++)
	{
		if(ch < MAX_UNIT_CHAN_COUNT)
			params.chan_mask_base |= 1 << ch;
		else
			params.chan_mask_exp  |= 1 << (ch - MAX_UNIT_CHAN_COUNT);
	}

	unsigned char* paramsPtr = (unsigned char*) &params;
	int paramsSize = sizeof(params);

	if(cjcChan)  // cjc command does not have wait and units
	{
		paramsPtr = (unsigned char*) &params.chan_mask_base;
		paramsSize = sizeof(params) - sizeof(params.wait) - sizeof(params.unit);
	}

	daqDev().queryCmd(cmd,  paramsPtr, paramsSize, (unsigned char*)fData, sizeof(float) * chanCount);
...
void AiETc32::check_TInArray_Args(int lowChan, int highChan, TempScale scale, TInArrayFlag flags, double data[]) const
{
...
void AiETc32::enableAllChannels()
{
...
void AiETc32::setMeasureMode(int mode)
{
...
void AiETc32::readCalDate()
{
	...
	if(getScanState() == SS_IDLE)
	{
		...
		bytesReceived = daqDev().queryCmd(CMD_FIELD_CAL_DATE_R,  NULL, 0, (unsigned char*)&fieldCalDateBuf, sizeof(fieldCalDateBuf));
...
void AiETc32::setCfg_ChanType(int channel, AiChanType chanType)
{
...
AiChanType AiETc32::getCfg_ChanType(int channel) const
{
	if(channel < 0 || channel >= mActualChanCount)
		throw UlException(ERR_BAD_AI_CHAN);
...
void AiETc32::setCfg_ChanTcType(int channel, TcType tcType)
{
	if(channel < 0 || channel >= mActualChanCount)
		throw UlException(ERR_BAD_AI_CHAN);
...
TcType AiETc32::getCfg_ChanTcType(int channel) const
{
	TcType tcTypeVal;
...
void  AiETc32::setCfg_OpenTcDetectionMode(int dev, OtdMode mode)
{
	if(dev >= 0 && dev < 2)
	{
...
OtdMode  AiETc32::getCfg_OpenTcDetectionMode(int dev) const
{
...
void AiETc32::setCfg_CalTableType(int dev, AiCalTableType calTableType)
{
	if(dev >= 0 && dev < 2)
	{
		TMEASURE_CFG config[2];
...
AiCalTableType AiETc32::getCfg_CalTableType(int dev) const
{
...
void AiETc32::setCfg_RejectFreqType(int dev, AiRejectFreqType rejectFreqType)
{
	if(dev >= 0 && dev < 2)
	{
		TMEASURE_CFG config[2];
...
AiRejectFreqType AiETc32::getCfg_RejectFreqType(int dev) const
{
...
unsigned long long AiETc32::getCfg_ExpCalDate(int calTableIndex)
{
...
void AiETc32::getCfg_ExpCalDateStr(int calTableIndex, char* calDate, unsigned int* maxStrLen)
{
	mDaqDevice.checkConnection();
