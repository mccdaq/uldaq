...
namespace ul
{
void DtFx2FwLoader::prepareHardware()
{
	FnLog log("DtFx2FwLoader::prepareHardware");
	...
	if(numDevs > 0)
	{
		...
		while ((dev = devs[devNum++]) != NULL)
		{
			...
			if(desc.idVendor == UsbDaqDevice::DT_USB_VID)
			{
				if(desc.idProduct == DaqDeviceId::DT9837_ABC_LD)
				{
					...
					if (status == LIBUSB_SUCCESS)
					{
						...
						if (status == LIBUSB_SUCCESS)
						{
							DtFx2FwLoader::downloadFirmware(devHandle, desc.idProduct);
...
int DtFx2FwLoader::downloadFirmware(libusb_device_handle* devHandle, unsigned int productId)
{
	int status = 0;
	unsigned short maxIntRam = 0;
	PINTEL_HEX_RECORD loaderImage;
	PINTEL_HEX_RECORD deviceImage;

	switch(productId)
	{
	case DaqDeviceId::DT9837_ABC_LD:
		loaderImage = DT_FX2LDR_FW_Image;
		deviceImage = DT9837A_FW_Image;
		maxIntRam = 0x1B3F;
		break;
	default:
		std::cout << "DtFx2FwLoader::downloadFirmware() error: unknown device" << std::endl;
		return -1;
	}
	...
	downloadIntelHex(devHandle, loaderImage, maxIntRam);
	reset8051(devHandle, 0);

	usleep(1000);

	downloadIntelHex(devHandle, deviceImage, maxIntRam);
...
int DtFx2FwLoader::downloadIntelHex(libusb_device_handle* devHandle, PINTEL_HEX_RECORD hexRecord, unsigned short maxIntRam)
{
	int status = 0;
	int sent;
	...
	while (ptr->Type == 0)
	{
		...
		{
			status = send(devHandle, ANCHOR_LOAD_EXTERNAL, ptr->Address, 0, ptr->Data, ptr->Length, &sent, 2000);
			if (status < 0)
				break;
		}
		ptr++;
...
int DtFx2FwLoader::reset8051(libusb_device_handle* devHandle, unsigned char reset)
{
	/*++
	...
	int sent;
	int status = send(devHandle, ANCHOR_LOAD_INTERNAL, CPUCS_REG_EZUSB, 0, &reset, sizeof(reset), &sent, 2000);

	status = send(devHandle, ANCHOR_LOAD_INTERNAL, CPUCS_REG_FX2, 0, &reset, sizeof(reset), &sent, 2000);

	return status;
}

/*
int DtFx2FwLoader::downloadFpga(libusb_device_handle* devHandle, unsigned short productId)
{
	int status = 0;
...
int DtFx2FwLoader::downloadFpgaImage(libusb_device_handle* devHandle, unsigned short nImage, unsigned char* pFpga, unsigned int FpgaLen)
{
	int status = 0;
...
int DtFx2FwLoader::send(libusb_device_handle* devHandle, uint8_t request, uint16_t wValue, uint16_t wIndex, unsigned char *buff, uint16_t buffLen, int* sent, unsigned int timeout)
{
...
int DtFx2FwLoader::query(libusb_device_handle* devHandle, uint8_t request, uint16_t wValue, uint16_t wIndex, unsigned char *buff, uint16_t buffLen, int* recevied, unsigned int timeout)
{
...
/*
int DtFx2FwLoader::set_interface(libusb_device_handle* devHandle)
{
	//FnLog log("UsbDaqDevice::send");
...
/*
bool DtFx2FwLoader::isFpgaLoaded(libusb_device_handle* devHandle)
{
...
/*
int DtFx2FwLoader::test(libusb_device_handle* devHandle)
{
	//FnLog log("UsbDaqDevice::query");
...
/*
void DtFx2FwLoader::readSerialNumber(libusb_device_handle* devHandle, libusb_device_descriptor descriptor, char* serialNum)
{
