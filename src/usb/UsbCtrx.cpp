...
#include "./dio/DioUsbCtrx.h"
#include "./ctr/CtrUsbCtrx.h"
...
UsbCtrx::UsbCtrx(const DaqDeviceDescriptor& daqDeviceDescriptor, std::string fpgaFileName) : UsbFpgaDevice(daqDeviceDescriptor, fpgaFileName)
{
	...
	setDaqIDevice(new DaqIUsbCtrx(*this));
	setDioDevice(new DioUsbCtrx(*this));
	setCtrDevice(new CtrUsbCtrx(*this, 4));
...
unsigned char UsbCtrx::getTrigModeCode(TriggerType type) const
{
	unsigned char code;
...
void UsbCtrx::setupTrigger(FunctionType functionType, ScanOption options) const
{
